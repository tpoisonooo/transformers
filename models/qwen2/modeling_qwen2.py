    ...
    def forward(
        ...
        **kwargs: Unpack[KwargsForCausalLM],
        ...
        if labels is not None:
            ...
            # 2) 算逐样本 loss（仅供记录，不建图）
            with torch.no_grad():
                # 对齐 ForCausalLMLoss 的 shift 逻辑
                ignore_index = -100
                labels = torch.nn.functional.pad(labels, (0, 1), value=ignore_index)
                shift_labels = labels[..., 1:].contiguous()          # (B, L-1)

                B, L_, V = logits.size(0), shift_labels.size(1), logits.size(-1)
                shift_logits = logits[:, :L_, :].contiguous()        # (B, L-1, V)

                # 展平，但保留 batch 维度的信息：view 成 (B, L-1, V)
                flat_logits  = shift_logits.view(-1, V)              # (B*(L-1), V)
                flat_labels  = shift_labels.view(-1)                 # (B*(L-1))

                # reduction='none' 得到每个 token 的 loss
                ...
                # 按 token 求平均得到逐样本 loss
                mask = shift_labels.ne(ignore_index)                 # (B, L-1)
                denom = mask.sum(dim=1, keepdim=True).clamp(min=1)
                domain_losses = ((token_loss * mask).sum(dim=1) / denom.squeeze(1)).detach().cpu()
